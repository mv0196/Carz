...
namespace Carz.AdvertisementService.Infrastructure.Handlers.Advertisement
{
    public class DisableAdHandler : IRequestHandler<DisableAdCommand,bool>
    {
        ...
        public async Task<bool> Handle(DisableAdCommand request, CancellationToken cancellationToken)
        {
            ...
            if(res)
            {
                _logger.LogInformation("Advertisement {AdvertisementId} disabled by {UserId}", request.Id, request.DisabledBy);
            }
            else
            {
                _logger.LogInformation("Unable to disable Advertisement {AdvertisementId} by {UserId}", request.Id, request.DisabledBy);
            }
            return res;
        }
    }
